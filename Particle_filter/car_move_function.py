...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        if abs(turning_angle) < 0.001:
            ...
            orientation_prime = (self.orientation + turning_angle) % (2 * pi)
        else:
            turning_radius = distance / turning_angle
            global_x = self.x - (sin(self.orientation) * turning_radius)
            global_y = self.y + (cos(self.orientation) * turning_radius)
            
            x_prime = global_x + sin(self.orientation + turning_angle) * turning_radius
            y_prime = global_y - cos(self.orientation + turning_angle) * turning_radius
            orientation_prime = (self.orientation + turning_angle) % (2 * pi)
        
        
        self.set(x_prime, y_prime, orientation_prime)
