...
def sense(p, Z, colors, sensor_right):
    ...
    for row in range(len(p)):
        ...
        for tile in range(len(p[row])):
            if colors[row][tile] == Z:
                d.append(p[row][tile] * sensor_right)
            else:
                d.append(p[row][tile] * (1-sensor_right))
        q.append(d)
        
    s = sum(sum(x) for x in q)
    for row in range(len(q)):
        for tile in range(len(q[row])):
            q[row][tile] /= float(s)
...
def move(p, U, p_move):
    ...
    for row in range(len(p)):
        ...
        for tile in range(len(p[row])):
            if U[0] == 0:
                change = p[row][(tile-U[1]) % len(p[row])] * p_move + (p[row][tile % len(p[row])] * (1-p_move))
            else:
                change = p[(row-U[0]) % len(p)][tile] * p_move + (p[row][tile % len(p[row])] * (1-p_move))
